...
For large document processing workloads we recommend using the asynchronous version of the API: `partition_file_async_submit`. The asynchronous API submits a file partitioning task to Aryn and returns with its `task_id`.
You can use the returned `task_id` to keep track of your request to partition the file using the `partition_file_async_result`. To learn more check out the [documentation](/sdk-reference/partition#partition-file-async-submit) and the [tutorial](/docparse/async_requests_tutorial)