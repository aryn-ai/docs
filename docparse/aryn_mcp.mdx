...
- Aryn API key - [Get one free here](https://app.aryn.ai/)
...
### Document Processing
```python
# Partition PDFs and extract structured data
partition_pdf("/path/to/document.pdf")

# Extract tables, images, and text with bounding boxes
get_boxes_drawn_on_pdf("/path/to/document.pdf")
```

### DocSet Management
```python
# Create and manage document collections
create_aryn_docset("my-docset", "Financial Documents")
add_aryn_document("my-docset", "/path/to/report.pdf")

# Search across your documents
search_aryn_docset("my-docset", "quarterly earnings")
query_aryn_docset("my-docset", "What were the key financial highlights?")
```

### Property Extraction
```python
# Extract custom properties from documents
extract_aryn_docset_properties("my-docset", [
    {"name": "revenue", "description": "Annual revenue figures"},
    {"name": "profit_margin", "description": "Profit margin percentages"}
])
```
