...
description: 'Diving deep into each of the chunking strategies supported by Aryn DocParse'
icon: 'brackets-curly'
---
...
The Maximize within limit strategy is meant to be used when you want to merge several consecutive elements together into a large chunk. Take the following example: 
...
the bounding boxes shown above are the result of calling DocParse without any chunking options specified. Let’s say for your question/answering RAG application, you want all the list items to be grouped together into 1 chunk to improve the quality of your embedding. Calling DocParse with the following chunking options will group the entire list into 1 chunk: 
