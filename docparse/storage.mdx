...
```python
from aryn_sdk.client.client import Client, SimpleQueryRequest

results = myClient.search(docset_id = [your-docset-ID], query = SimpleQueryRequest(query=[your-query-phrase], query_type=[KEYWORD|VECTOR], return_type=[DOC|ELEMENT]))
```

this will return a SimpleQueryResponse object as follows:

```
SimpleQueryResponse(results=[], query_embedding=None, next_page_token=None)
```

the results parameter will be a list of either elements or documents that match the search query. To learn more about the search API please reference the sdk documentation [here](../sdk-reference/store.mdx).
