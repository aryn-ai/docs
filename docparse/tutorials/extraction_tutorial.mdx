...
Here's an example of a property with a regex validator expecting phone numbers from the US. Currently, only regex validators are supported.
...
origin_phone_number_property = {
    ...
    "type": {
        "type": "string",
        "description": "The phone number of the shipment origin",
        "validators": [
            {
                "type": "regex",
                "regex": "^\\(\\d{3}\\) \\d{3}-\\d{4}$"
...
Try adding an email validator to extract the dev
