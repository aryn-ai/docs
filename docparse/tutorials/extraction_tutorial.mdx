...
In this tutorial, we'll use the Aryn SDK to extract properties from a [document](https://aryn-public.s3.us-east-1.amazonaws.com/extraction-tutorial/gri.pdf).

By the end of this tutorial, you will be able to extract properties from a document as complicated as the one below:
...
2. Open up the follow-along Colab notebook and copy in your API key by clicking the key icon on the leftmost side of the screen.
   ...
   Alternatively, you can set the API key as an environment variable if you are doing this locally instead of on Colab.
...
Now let's also extract the submission contact email property from the document.  For reference, this document has contact info in a table at the end of the last page.  As mentioned in the [table extraction tutorial](../tutorials/tables), we can use the `table_mode` parameter to extract table structure. Since the information is in a table, let's set `table_mode="standard"` to extract the table structure.
...
Now that we know how to extract objects, let's extract an array of them. You can also extract arrays of other types such as to extract a list of strings or numbers, but here we want to group related values together. Looking at the table in the middle of page 2, we see exposure broken down by US State.
