...
In this [example](https://colab.research.google.com/drive/1_OOvmy-N5qQNxKe-S2vKHBodfcBR25hg), 
we'll use DocParse to extract images from a 
[battery manual](https://aryn-public.s3.amazonaws.com/partitioner-blog-data/powerwall_parts_boards.pdf). 
We'll go through the important code snippets below to see what's going on.

Let's focus on the following code that calls the DocParse [partition](../../api-reference/introduction) API
...
If you inspect the returned `partitioned_file`, you'll notice that it's a large JSON object with 
