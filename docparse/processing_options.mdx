...
orders, contracts, etc.  Currently, the only option allowed is `schema` which describes properties in the document
being processed that you want DocParse to extract, e.g. the total dollar amount in an invoice.
