...
### table_mode
A string specifying table structure extraction mode. `Default: "none"'. Valid options are
...
for their structure; as a result, table cells and their bounding boxes are not included in the response. `Default: False`.
...
This can be useful for working with tables that have missing or misaligned text. `Default: False`.
...
A boolean that specifies whether to remove line breaks from the text. `Default: False`.
