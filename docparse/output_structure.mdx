...
## Properties Format
If `property_extraction_options: {'schema': [{...}]}` is passed in the request, the output will contain a top-level `properties` element which will contain `entity` which is a dictionary of key value pairs where each key maps to a property in the schema and its value is the result of extraction from the document.
