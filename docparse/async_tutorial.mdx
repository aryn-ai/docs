...
for i, file_name in enumerate(files):
  try:
      task_ids[i] = partition_file_async_submit(open(file_name, 'rb'))["task_id"]
...
```python
f = open("/path/to/my/file.docx", "rb")
partition_file_async_submit(f, webhook_url="https://example.com/alert")
```
