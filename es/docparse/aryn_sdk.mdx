...
`partition_file`acepta las mismas opciones que curl, excepto como argumentos de palabras clave. Puedes encontrar una lista de opciones[aquí](./processing_options).
...
`aryn_sdk`proporciona algunas funciones auxiliares para facilitar el trabajo y la visualización de la salida de`partition_file`más fácil.
...
El soporte para fragmentación se ha añadido en v0.1.9. Puedes habilitar las opciones de fragmentación predeterminadas especificando un diccionario vacío:
...
Las opciones completas de fragmentación están documentadas[aquí](/docparse/chunking_strategies).
...
Puedes usar el`task_id`devuelto para hacer seguimiento de tu solicitud de particionamiento del archivo usando el`partition_file_async_result`. Para aprender más, consulta la[documentación](/sdk-reference/partition#partition-file-async-submit)y el[tutorial](/docparse/tutorials/async_requests_tutorial).
