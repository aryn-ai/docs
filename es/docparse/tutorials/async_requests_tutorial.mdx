...
Las siguientes funciones se pueden usar juntas para enviar archivos a DocParse para particionar de forma asíncrona:
...
Es importante tener en cuenta que enviar múltiples tareas de particionamiento asíncronas al mismo tiempo no garantiza que se ejecuten simultáneamente.

## Usando un webHook

Opcionalmente, también puede establecer un webhook para que los servicios de Aryn lo llamen cuando su tarea esté completada:
...
Si desea listar todas las tareas partition\_file asíncronas que se están ejecutando y aún no se han completado en su cuenta, puede llamar a la siguiente función:
...
Si desea cancelar una tarea partition\_file asíncrona específica, puede llamar a la siguiente función:
...
Puede consultar su resultado con el comando a continuación. Use el ID de llamada que el comando devolvió como salida.
