...
description: Extrae datos estructurados de tablas de PDFs usando Aryn DocParse
icon: table
---

En[este ejemplo](https://colab.research.google.com/drive/1Qpd-llPC-EPzuTwLfnguMnrQk0eclyqJ?usp=sharing), usaremos DocParse para extraer una tabla de flujo de efectivo (mostrada abajo) del documento financiero 10-K de 3M, y convertirla en un dataframe de pandas.

<Frame>
  <img src="../../images/3m_10k_extract.png" />
</Frame>

## Extrayendo Celdas de Tabla de DocParse
...
Si inspeccionas la variable partitioned\_file, notarás que es un objeto JSON grande con detalles sobre todos los componentes en el PDF (consulta[esta página](../output_structure) para entender el esquema del objeto JSON devuelto en detalle). A continuación, destacamos el`table` elemento que contiene la información sobre la tabla en la página.
...
En particular, veamos el`cells` campo que es un array de objetos de celda que representan cada una de las celdas en la tabla. Centrémonos en el primer elemento de esa lista.
...
Aquí hemos detectado la primera celda, su cuadro delimitador (que indica las coordenadas de la celda en el PDF), si es una celda de encabezado y su contenido. Puedes procesar este JSON como desees para análisis adicional. En[el notebook](https://colab.research.google.com/drive/1Qpd-llPC-EPzuTwLfnguMnrQk0eclyqJ?usp=sharing) usamos la`tables_to_pandas` función para convertir el JSON en un dataframe de pandas y luego realizar algunos análisis en él:
