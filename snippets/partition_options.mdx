...
which tries to extract the embedded text elementwise and falls back to performing OCR otherwise. The other options are
`inline`, which extracts embedded text; `ocr_standard`, which uses the classical OCR pipeline; and `ocr_vision`, which
uses a vision model for OCR. Note that `ocr_vision` is only available for PAYG users only. These text_modes have been
deprecated: `standard`, `fine_grained`, `standard_ocr`, and `vision_ocr`.
