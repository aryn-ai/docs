...
which extracts the embedded text when available and elementwise falls back to performing ocr when there is no embedded
text. The other options are `inline`, which extracts embedded text; `ocr_standard`, which uses the classical OCR
pipeline; and `ocr_vision`, which uses a vision model for OCR.
Note that `ocr_vision` is only available for PAYG users only. These text_modes have been deprecated: `standard`,
`fine_grained`, `standard_ocr`, and `vision_ocr`.
...
- `use_ocr` (deprecated): A boolean value that, when set to `True`, causes DocParse to extract text using an OCR model. This is
