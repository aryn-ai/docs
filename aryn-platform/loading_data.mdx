...
When adding documents to your DocSet, Aryn automatically parses and processes them using its compound AI model [DocParse technology](https://www.aryn.ai/docparse). This includes segmentation, OCR, table extraction, image summarization, chunking, vector embedding, and more. 

Once a document is added to an Aryn DocSet, Aryn will store an it using an internal structured representation called [Document](docparse/output_structure#json-format). This is the same output format of DocParse and how documents are stored in [Sycamore DocSets](https://sycamore.readthedocs.io/en/stable/sycamore/APIs/document.html). This structured representation contains a list of Elements (images, tables, text, etc.) found within the document as well as a map of document Properties.

Processed documents are structured in the Document format, and added to your DocSet.
...
You can use the [Add Document function](https://docs.aryn.ai/sdk-reference/storage#add-document) to add a doc to your DocSet using the Aryn SDK. This is an asynchronous function.


**Using Sycamore document ETL**

You can write to an Aryn DocSet using [Sycamore](https://github.com/aryn-ai/sycamore), an open source document ETL framework. This is helpful if you require custom processing, enrichment, or chunking for your documents. For more information on using Aryn as a target for a Sycamore pipeline, [click here](aryn-platform/data-preparation/sycamore_document_etl).
