...
At the bottom of the query box in the Workspaces UI, you'll find a toggle for `Deep Analytics`. When enabled, Aryn will generate a query plan for your query, which you can then run or edit before executing. If the toggle is not enabled, Aryn will use RAG instead — this is ideal for simpler queries that do not require complex query plans and this could also be useful for querying a small set of documents.
