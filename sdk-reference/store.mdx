...
- `filters`: A string that specifies a boolean expression. This expression specifies the condition to use when extracting documents or elements from the docset.
  * 'windSpeed>5 AND state="Texas"'
...
results = myClient.search(docset_id = [your-docset-ID], query = SearchRequest(query=[your-query-phrase], query_type=[KEYWORD|VECTOR], return_type=[DOC|ELEMENT]))
...
- results: A list containing either the documents or elements that matched the search criteria and were returned.
- query_embedding: If the query_type selected is 'vector', this is field indicates the embedding vector that was used to perform the search. 
