...
<AccordionGroup>
<Accordion title="Parameters">
- Note `partition_file_async_submit` accepts all parameters `partition_file` accepts in the same order and in addition
includes the parameters below:
...
- `async_submit_url`: str. If `async_submit_url` is specified, `partition_file_async_submit` will submit async requests
to this endpoint. When `async_submit_url` is not specified, `partition_file_async_submit` will intrepret the specified
`docparse_url` or `aps_url` parameter as the synchronous endpoint and translate it to the proper asynchronous one. If
no urls are specified, then it will use the default async Aryn endpoint.
</Accordion>

<Accordion title="Return Value">
A dict like `{"job_id": "aryn:j-yjchiesfqlish1ye58cco41"}` containing the job_id of the submitted request.
</Accordion>
...
<AccordionGroup>
<Accordion title="Parameters">
- `job_id` Required. str of the job id to cancel
...
- `ssl_verify` A `boolean` that controls whether the client verifies the SSL certificate of the chosen DocParse server.
In Databricks, set this to `False` to fix SSL incompatibilities. ssl_verify is `True` by default, enforcing SSL
verification.
- `async_result_url` url of the endpoint to submit the get result request to
</Accordion>
<Accordion title="Return Value">
A dict like the below containing "status" and "status_code". When "status" is "done", the returned dict also contains
"result" which contains what would have been returned had `partition_file` been called directly. "status" can be
"done", "pending", "error", or "no_such_job".<br />
```json
{
  "status": "done",
  "status_code": 200,
...
In Databricks, set this to `False` to fix SSL incompatibilities. ssl_verify is `True` by default, enforcing SSL
verification.
- `async_cancel_url` url of the endpoint to submit the cancellation request to
</Accordion>
<Accordion title="Return Value">
A `bool` which is True on success. Asynchronous jobs may only be successfully cancelled once. Once a job has been
cancelled, any `partition_file_async_result` calls using that job's id will return with a status of "no_such_job" soon
after though not immediately.
</Accordion>
<Accordion title="Example">
```python
...
- `aryn_config`: An ArynConfig object (defined in [aryn_sdk/config.py](https://github.com/aryn-ai/sycamore/blob/main/lib/aryn-sdk/aryn_sdk/config.py)), used for finding an api key. If aryn_api_key is set it will override this. The default ArynConfig looks in the env var `ARYN_API_KEY` and then in the file ~/.aryn/config.yaml. Default is None (aryn-sdk will look in the aryn_api_key parameter, in your environment variables, and then in ~/.aryn/config.yaml).
- `ssl_verify` A `boolean` that controls whether the client verifies the SSL certificate of the chosen DocParse server. In Databricks, set this to `False` to fix SSL incompatibilities. ssl_verify is `True` by default, enforcing SSL verification.
- `async_list_url` url of the endpoint to submit the list jobs request to
...
A dict containing "jobs" which is a dict containing jobs with their job_id as their key and their value is a dict
containing the keys "path" and "state".

An example reponse body is shown below:
