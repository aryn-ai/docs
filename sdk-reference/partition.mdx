...
- `chunking_options`: A dictionary of options for specifying chunking behavior. Chunking is only performed when this option is present, and default options are chosen when `chunking_options` is specified as `{}`.
    ...
    - `merge_across_pages`: A boolean that, when True, the selected chunker will attempt to merge chunks across  pages boundaries. Does not apply to the 'marked' merger, which never merges across pages. Defaults to `True`. 
