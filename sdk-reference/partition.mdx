...
<Accordion title="Return Value">
A dict like the one in the example below containing "status". When "status" is "done", the returned
dict also contains "result" which contains what would have been returned had `partition_file` been called directly. If there is an error with partitioning the file itself, then the "status" will still be "done" but the
"result" will contain an "error" field indicating what went wrong.

"status" can be "done" or "pending". <br />
```json
{
  "status": "done",
  "result": ...
