...
"result" will contain an "error" field. Unlike `partition_file`, `partition_file_async_result` does
not raise a `PartitionError` on failure to partition.
```json
{
  "status": "done",
  "status_code": 200,
...
```
| `status` | `status_code` | `result` present? |
| -------- | ------------- | ----------------- |
| "done" | 200 | yes |
| "pending" | 202 | no |
| "no_such_job" | 404 | no |
