...
A `bool` which is true on sucess. Asynchronous jobs may only be sucesfully cancelled once. Once a job has been cancelled, partition_file_async_result calls using that job's id will return with a status of "no_such_job" soon after though not immediately.
