...
A `bool` which is true on sucess. Asynchronous jobs may only be sucesfully cancelled once. Once a job has been cancelled, partition_file_result_async calls against that job's id will return JobStatus.NO_SUCH_JOB shortly though not immediately.
