...
<Accordion title="Parameters">
- Note `partition_file_submit_async` accepts all parameters `partition_file` accepts in the same order and in addition includes the parameters below:
...
- `force_async_url`: bool not for normal use. `partition_file_submit_async` automatically changes the specified DocParse API url to the async endpoint, unless `force_async_url` is set to True. When False, `partition_file_submit_async` will use the exact `docparse_url` or `aps_url` provided.
