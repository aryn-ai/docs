...
- `threshold`: A `float` between `0.0` and `1.0`, inclusive, which serves as a cutoff to determine which bounding boxes in the file are returned. 
...
User errors:
- ```HTTPError: Error:status_code 400```. Reason: `"This async API requires you to upgrade your account plan"`
...
- ```HTTPError: Error:status_code 400```. Reason: `"Invalid Aryn API key"`
    - Fix: Please provide a valid API key either as a parameter or specify it in the environment variable `ARYN_API_KEY`.
- ```HTTPError: Error:status_code 400```. Reason: `"Expired Aryn API key"`
...
User errors:
- ```HTTPError: Error:status_code 400```. Reason: `{"detail": "This async API requires you to upgrade your account plan"}`
    - Fix: Please upgrade your account [here](https://console.aryn.ai/billing)
- ```HTTPError: Error:status_code 403```. Reason: `{"detail": "No Aryn API Key provided"}`
    - Fix: Please provide an API key either as a parameter or specify it in the environment variable `ARYN_API_KEY`.
- ```HTTPError: Error:status_code 400```. Reason: `{"detail": "Invalid Aryn API key"}`
    - Fix: Please provide a valid API key either as a parameter or specify it in the environment variable `ARYN_API_KEY`.
- ```HTTPError: Error:status_code 400```. Reason: `{"detail": "Expired Aryn API key"}`
    - Fix: Please get a new API key [here](https://console.aryn.ai/api-keys). 
